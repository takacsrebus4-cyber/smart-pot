...
WiFiClient client_for_data_upload;
WiFiClient client_for_arduino_registration;
...
void setup() {
  ...
  mac_address = String(mac[5], HEX) + ":" + String(mac[4], HEX) + ":" + String(mac[3], HEX) + ":" + String(mac[2], HEX) + ":" + String(mac[1], HEX) + ":" + String(mac[0], HEX);
...
void loop() {
  ...
  if (timer >= 900000) {
    upload_data(light_intensity, mappedSoilMoisture, temperature, humidity, light_amount, mac_address);
    timer = 0;
    light_amount = 0;
  }
}
...
void upload_data(float light_intensity, int moisture, float temperature, float humidity, float light_amount, String mac_address) {
  ...
  if (client_for_data_upload.connect(server, port)) {
    ...
    client_for_data_upload.println("Connection: close");
    client_for_data_upload.print("Content-Length: ");
    ...
    read_response(client_for_data_upload);

    client_for_data_upload.flush();
...
void registerArduino(String json_mac_address){
  Serial.println("\nStarting connection to server...");

  if (client_for_arduino_registration.connect(server, port)) {
    Serial.println("connected to server");

    Serial.println(json_mac_address);

    // Make a HTTP request:
    client_for_arduino_registration.println("POST /registerArduino HTTP/1.1");
    client_for_arduino_registration.println("Host: " + String(server));
    client_for_arduino_registration.println("Content-Type: application/json");
    client_for_arduino_registration.println("Connection: close");
    client_for_arduino_registration.print("Content-Length: ");
    client_for_arduino_registration.println(json_mac_address.length());
    client_for_arduino_registration.println();

    // Send JSON body
    client_for_arduino_registration.println(json_mac_address);

    Serial.println("Request sent");
    Serial.println("Waiting for response...");
    read_response(client_for_arduino_registration);

    client_for_arduino_registration.flush();
    client_for_arduino_registration.stop();
